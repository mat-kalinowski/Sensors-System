                               ...
                               int32_t event_id, void *event_data)
{
    ...
    ESP_LOGI(mac_string, "Wi-Fi disconnected, trying to reconnect...");
    ...
    if(s_retry_num == 0){
        ESP_LOGI(mac_string, "WiFi provisioning timeout, falling back to smartconfig...");
...
static void sc_callback(smartconfig_status_t status, void *pdata)
{
    switch (status) {
        case SC_STATUS_FIND_CHANNEL:
            ESP_LOGI(mac_string, "smartconfig: waiting for configuration");
            break;

        case SC_STATUS_GETTING_SSID_PSWD:
            ESP_LOGI(mac_string, "smartconfig: decoding ssid and password");
            break;

        case SC_STATUS_LINK: ;
            wifi_config_t *wifi_config = pdata;

            ESP_LOGI(mac_string, "SSID:%s", wifi_config->sta.ssid);
            ESP_LOGI(mac_string, "PASSWORD:%s", wifi_config->sta.password);
...
static void smartconfig_task(void * parm)
{
    ...
    while (1) {
        ...
        if(uxBits & CONNECTED_BIT) {
            ESP_LOGI(mac_string, "WiFi connected to access point");
        }

        if(uxBits & ESPTOUCH_DONE_BIT) {
            ESP_LOGI(mac_string, "smartconfig: WiFi credentials configured, shutting off...");
